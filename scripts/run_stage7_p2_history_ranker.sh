...
usage() {
  cat <<'EOF'
Run P2-3: history-aware learned reranker.
...
Environment:
  ...
  OUT_DIR       Default: ${ROOT}/p2_history_ranker
  EPOCHS        Default: 18
  MAX_NEGATIVES Default: 80
  SEED          Default: 20260706

Dry run:
  DRY_RUN=1 bash scripts/run_stage7_p2_history_ranker.sh

Actual run on AutoDL:
  CUDA_VISIBLE_DEVICES="" bash scripts/run_stage7_p2_history_ranker.sh
EOF
}
...
OUT_DIR="${OUT_DIR:-${ROOT}/p2_history_ranker}"
LOG_DIR="${ROOT}/logs"
LOG_FILE="${LOG_DIR}/stage7_p2_history_ranker.log"

CF_VERSION_DIR="data/Amazon/sid_versions/cf_k512_dedup/${CATEGORY}"
CF_TRAIN_CSV="${CF_TRAIN_CSV:-${CF_VERSION_DIR}/train.csv}"
CF_ITEM_EMB="${CF_ITEM_EMB:-data/Amazon/cs_embeddings/${CATEGORY}/${CATEGORY}.cf_emb.npy}"
CF_ROW_INDEX="${CF_ROW_INDEX:-data/Amazon/cs_embeddings/${CATEGORY}/${CATEGORY}.row_index.json}"
...
FEATURE_WEIGHTS_CSV="${OUT_DIR}/feature_weights.csv"
HISTORY_JSONL="${OUT_DIR}/history_reranked_candidates.jsonl"
...
required = [
    "history_reranked_candidates.jsonl",
...
with open(out_dir / "history_reranked_candidates.jsonl", "r", encoding="utf-8") as f:
    rows = sum(1 for line in f if line.strip())
if rows != split_summary.get("num_samples"):
    raise SystemExit(f"history rows mismatch: {rows} vs {split_summary.get('num_samples')}")
...
print(f"existing_ok selected={selected} decision={decision} valid_select_history_hr20={learned} heuristic_hr20={heuristic}")
PY
}
...
echo "seed=${SEED}"
echo "train_csv=${CF_TRAIN_CSV}"
echo "item_emb=${CF_ITEM_EMB}"
echo "row_index=${CF_ROW_INDEX}"
...
for path in "$CF_TRAIN_CSV" "$CF_ITEM_EMB" "$CF_ROW_INDEX"
do
  ensure_no_test_path "$path"
  check_file "$path"
done
...
minimal_decision="$(json_value "$P2_MINIMAL_REPORT_JSON" selection.freeze_recommendation.decision)"
[[ "$minimal_decision" == "do_not_freeze_p2_minimal_ranker" ]] || fail "P2-2 minimal ranker decision must be do_not_freeze_p2_minimal_ranker, got: $minimal_decision"
...
if [[ "$FORCE" != "1" && -e "$RANKER_REPORT_JSON" ]]; then
  echo "===== Existing P2-3 outputs ====="
  if validate_existing_outputs; then
    echo "Reuse P2-3 history-aware ranker."
    echo "Summary: $RANKER_REPORT_MD"
    exit 0
  fi
  fail "Existing P2-3 outputs are incomplete or invalid. Set FORCE=1 to regenerate."
fi
...
if [[ "$DRY_RUN" == "1" ]]; then
  echo "Dry run only. No P2-3 outputs were written."
  exit 0
fi

"${cmd[@]}"

echo "===== Validate P2-3 outputs ====="
validate_existing_outputs

echo
echo "Stage 7 P2-3 history-aware ranker completed."
echo "Summary: $RANKER_REPORT_MD"
